...
if(isset($_GET['remove']) && $_GET['remove'] == "y" && isset($_GET['id']) && is_numeric($_GET['id'])){
	...
	$res = $dbsite->db->query($sqltext); # or die("Cant delete diary items");
...
$sqltext = "SELECT diaryid,diary.title,diary.content,diary.incept,diary.done,diary.location,diary.staffid,diary.every,diary.end,fname,sname
			FROM diary,staff

...
###########################################################
# 
# /srv/ath/adm/phpStartPoint.pl athcore && /srv/ath/adm/phpStartPoint.pl athdb100
...
";
	...
	open( FH, ">$libdir/obj/$capTableName.php" );
	print FH '<?php' . "\n" . $bindValues . $phpClass . "
?>";
	close(FH);
...
my $outFile = 'AthenaCore.php';
if ($db eq 'athdb100'){
	$outFile = 'AthenaSite.php';
	$allPHPClasses=~s/\$db/\$dbsite/gs;
	$allPHPClasses=~s/usr\), 'd/usr), 's/gs;
	$allPHPClasses=~s/usr\), 'i/usr), 's/gs;
}else{
	$allPHPClasses=~s/\$db/\$dbsys/gs;
}


open( FH, ">$libdir/$outFile" );
...
sub makeDirectory {
	...
	my $userdir = '/srv/ath/src/pub';
	chomp $userdir;
	
	$dir = $userdir . '/phpstartpoint/'.$db;
